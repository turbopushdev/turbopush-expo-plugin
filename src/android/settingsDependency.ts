...
function applySettings(gradleSettings: string) {
  const codePushSettings = `\ninclude ':react-native-code-push'\nproject(':react-native-code-push').projectDir = new File(["node", "--print", "require.resolve('@turbopush/react-native-code-push/package.json')"].execute(null, rootDir).text.trim(), "../android/app")`
